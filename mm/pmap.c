...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
	 ...
	 * entry value. */
	pgdir_entryp = pgdir + PDX(va); //get Page Directory -- Virtual Address
	/* Step 2: If the corresponding page table is not exist and parameter `create`
	 * is set, create one. And set the correct permission bits for this new page
	 * table. */
	pgtable = (Pte*)KADDR(PTE_ADDR(*pgdir_entryp));
	if ((*pgdir_entryp & PTE_V) == 0) { // if there's no Page Table
		if (create) {
			pgtable = (Pte*)alloc(BY2PG, BY2PG, 1); //get a new Page -- Physical Address
			*pgdir_entryp = PADDR(pgtable); // record its Address -- Virtual Address
			*pgdir_entryp |= PTE_V | PTE_R; // set permission: PTE_V | PTE_R
		} else return 0;
	}
	/* Step 3: Get the page table entry for `va`, and return it. */
	pgtable_entry = pgtable + PTX(va); // get Page Table -- Virtual Address
...
int inverted_page_lookup(Pde *pgdir, struct Page *pp, int vpn_buffer[]) {
	...
	for (i = 0,np = 0;i <= 0x2fff0000;i+=4096) {
	//for (i = 0;i < pp->num;i++) {
