...
void sys_yield(void)
{
	...
	new = (struct Trapframe *)(TIMESTACK - sizeof(struct Trapframe));
	bcopy((void*) old, (void*) new, sizeof(struct Trapframe));
	sched_yield();
}
...
int sys_write_dev(int sysno, u_int va, u_int dev, u_int len)
{
        // Your code here
	u_int det = dev + len;
	if (!( dev >= 0x10000000 && det <= 0x10000020 ||
	       dev >= 0x13000000 && det <= 0x13004200 ||
	       dev >= 0x15000000 && det <= 0x15000200)) {
		return -E_INVAL;
	}
